...
```text
topic approved -> research -> script approved -> V4 assets -> voice -> render -> local QC -> ChatCut polish -> publish approval
```
...
ChatCut 是本地 V4 master 的后处理与人工审阅层，不是项目真源。每次精修需新建 `v4-chatcut-<revision>`，记录 ChatCut project id、导入 asset id、修改清单、导出路径和审核人。不得覆盖 `10_delivery_交付/v4/` 的本地 master。
