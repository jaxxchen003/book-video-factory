...
这是一个可安装的 Codex Skill，用于运行“权利可审计、本地母版优先”的中文图书短视频工作流。它同时支持稳定的双语编辑模板和 9:16 编辑型纸拼贴讲解风格，并将资料证据、生成来源、人工审批、成本和发布决定留存在可复核的文件中。
...
| 风格 | 输出 | 适用场景 | 核心素材契约 |
| --- | --- | --- | --- |
| `book-editorial-bilingual-v2` | 3:4 双语编辑型母版，可生成 9:16 衍生版 | 书封驱动、包装稳定、可批量复用的图书笔记 | 有授权的真实书封、12 张已审批静帧、本地旁白、双语字幕、确定性 FFmpeg 合成 |
| `paper-collage-explainer-v1` | 9:16 编辑型纸拼贴知识讲解 | 用短镜头和视觉隐喻解释高信息密度概念 | 每个概念 4–8 秒；Gate 1 审隐喻、Gate 2 审静帧/联系表、Gate 3 审生成片段；音频和字幕在本地母版统一完成 |

选择新风格前，请阅读完整的 [`paper-collage-explainer-v1` 工作流](skills/book-video-factory/references/paper-collage-explainer.md)。

### Gemini API 与 Google Flow 如何选择

纸拼贴风格提供两条明确的画面生成路线。无论选择哪条，旁白定时、字幕、BGM、SFX、QC、manifest 和最终母版都由本地图书视频工厂负责。

| 路线 | 适用情况 | 依赖与边界 |
| --- | --- | --- |
| Gemini API | 需要可复现的程序化生成、operation 追踪或批量处理 | 用户授权的 `GEMINI_API_KEY`、当前 Google Gen AI SDK，以及已批准的配额与费用。默认使用 Gemini Omni Flash；需要首尾帧控制或 Veo 延长时使用 Veo 3.1。密钥不得写入提示词、日志或 manifest。 |
| Google Flow | 导演希望在 Google 创作界面逐条尝试镜头 | 合格的 Google AI 订阅、可用 credits、桌面 Chromium 和用户合法导出的文件。Flow 在本 Skill 中是人工 UI 路线，不假设存在可编程 Flow API。 |
...
1. 锁定可归因资料和简洁中文脚本。
2. 将旁白拆成每段 4–8 秒、一个概念对应一个画面单元。
3. Gate 1：审批“概念 → 可见隐喻 → 动画动作 → 禁止项”。
4. Gate 2：审批首尾帧或联系表。
5. Gate 3：通过 Gemini API 或 Google Flow 生成，检查时长、画幅、水印、文字污染、动作连续性和可解码性。
6. 将通过的片段规范化为静音 9:16 H.264 素材。
7. 以已批准旁白和 ASR 时间为真值建立本地时间线，再统一添加字幕、已授权 BGM、SFX 和品牌层。
8. 运行技术 QC，并把画面、BGM、本地母版、权利和发布决定绑定到不可变哈希。
...
[`paper-collage-explainer-v1` · 播放视频 · 82.7 秒 · 9:16](https://jaxxchen003.github.io/book-video-factory/demos.html#chaoyue-baisui)

这个网页预览来自用户审批通过的 r5 本地母版。原母版保持不可变；仓库展示版仅做了网页压缩，并替换了开头的真实书封区域。详细来源和哈希见[案例 provenance manifest](examples/manifests/chaoyue-baisui-paper-collage.json)。案例公开展示不等于授权他人复用图书、书名、译文、音乐或任何第三方元素。
...
| 纸拼贴视频片段 | Gemini API 的 Gemini Omni Flash/Veo 3.1，或用户操作的 Google Flow | 需要用户授权、成本/credits 审批，以及不可变的提示词与 operation/导出来源记录 |
